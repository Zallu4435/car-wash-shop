...
import { useCreatePoster } from '@/api/domains/admin-marketing/queries';
...
export default function NewPosterPage() {
  ...
  const onSubmit = async (data: PosterFormInput) => {
    try {
      await createPosterMutation.mutateAsync(data);
        ...
        <CardHeader className="pb-3 sm:pb-4">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              <CardTitle className="text-base sm:text-lg md:text-xl">Create New Poster</CardTitle>
              <p className="text-xs sm:text-sm text-muted-foreground mt-0.5 sm:mt-1">Upload a promotional poster for the landing page</p>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6">
            ...
            {/* Submit Button */}
            <Button type="submit" className="w-full h-10 sm:h-11 text-xs sm:text-sm border-2" disabled={isSubmitting || createPosterMutation.isPending}>
              <Plus className="mr-1.5 sm:mr-2 h-4 w-4 sm:h-5 sm:w-5" />
              {isSubmitting || createPosterMutation.isPending ? 'Creating...' : 'Create Poster'}
            </Button>
          </form>
        </CardContent>
      </Card>
