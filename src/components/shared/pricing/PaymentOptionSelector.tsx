  ...
  return (
    <RadioGroup value={value} onValueChange={onChange} className="space-y-2.5 sm:space-y-3">
      {options.map((option) => (
        <Card
          key={option.id}
          className={`cursor-pointer transition-all duration-200 border-2 hover:border-primary/50 hover:shadow-md ${value === option.id
        ...
        >
          <div className="p-3 sm:p-4 flex items-start gap-3 sm:gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              >
                <div className={`p-1 sm:p-1.5 rounded-lg flex-shrink-0 ${value === option.id ? 'bg-primary/10' : 'bg-muted'
                  }`}>
                  {option.icon}
                </div>
